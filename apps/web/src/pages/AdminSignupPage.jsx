      ...
      <div className="w-full max-w-md">
        <div className="bg-white border border-ink/10 p-8">
          ...
          <button onClick={handleGoogleLogin}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-ink/15 text-ink hover:bg-cream transition-colors mb-6">
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="px-3 bg-white text-ink/40 eyebrow">OR EMAIL</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            <div>
              ...
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name" required className="border-ink/15 text-ink" />
            </div>
            <div>
            ...
            <div>
              ...
              <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min 6 characters" required className="border-ink/15 text-ink" />
            </div>
            <div>
              <label className="block text-xs eyebrow text-ink/60 mb-1">CONFIRM PASSWORD</label>
              <Input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="••••••••" required className="border-ink/15 text-ink" />
            </div>
            <Button type="submit" className="w-full bg-ink text-cream hover:bg-ink/90" disabled={isLoading}>
          ...
          <p className="text-center text-ink/60 text-sm mt-6">
            ...
            <Link to="/admin/login" className="text-rouge hover:text-wine underline">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
